#!/usr/bin/env python3
# generate_emitters_cob.py • COB + perimeter strip macro emitters (Lambertian)
#
# Outputs:
...
#   - Strips are Lambertian rectangular area emitters placed on each ring's perimeter.
#   - Strips are NOT solver-controlled in the default workflow; they are emitted as a fixed fraction
...
COB_HEATSINK_DIAM_IN_DEFAULT = 5.0  # mechanical placement model (for strip/frame placement)
...
# Strips: KingBrite W55 style (modeled as emitting rectangles)
STRIP_W_M = 0.055
...
# Strip power is proportional to COB power per ring (supplemental spectrum, not uniformity driver).
COB_STRIP_W_FRACTION_DEFAULT = 0.05  # strip watts = fraction * COB ring total watts
...
def _compute_positions_from_env() -> Tuple[List[Dict[str, float]], float, Dict[str, Any]]:
    ...
    if mode == "rect_rect":
        # Use rect_layout UV rings, but keep pitch pinned (does not stretch-to-fit).
        ...
        # Rect UV pattern lives on an even-parity lattice (Δu=2, Δv=2 for adjacent points).
        fixture_w = 2.0 * pitch_axis
        fixture_h = 2.0 * pitch_axis
        xs = [p["x"] for p in fixtures] if fixtures else [0.0]
        ys = [p["y"] for p in fixtures] if fixtures else [0.0]
        meta = {
...
def _build_ring_perimeter_strips(fixtures: List[Dict[str, float]], meta: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    min_ring = max(1, _env_int("COB_STRIP_MIN_RING", STRIP_MIN_RING_DEFAULT))
...
def _write_strip_overlay_rad(strips: List[Dict[str, Any]], *, out_path: Path) -> None:
    ...
    with out_path.open("w") as fh:
        fh.write("# Strip overlay geometry (non-emitting)\n\n")
...
def generate_emitters_cob(
    *,
    ...
    """
    Build COB + strip-ring emitters.

    COBs are driven by per-ring watts-per-COB (solver output). Strips are proportional to
    ...
    # Strip geometry (per-ring rectangles) first, so we can compute per-ring strip length.
    ...
    # Strips: derived from COB ring watts unless explicitly overridden.
    ...
    # Glow maxrad must be >0 to behave as a light source.
    room_L = float(meta.get("room_L_m", 0.0))
    room_W = float(meta.get("room_W_m", 0.0))
    z_emit = float(meta.get("z_emit_m", 0.6096))
    glow_maxrad = max(2.0, math.sqrt(room_L * room_L + room_W * room_W + z_emit * z_emit))

    strip_rad_by_ring: Dict[int, float] = {}
    for L in range(rings_local):
        total_w = float(strip_ring_total_w.get(L, 0.0))
        total_len = float(strip_len_by_ring.get(L, 0.0))
        # Lambertian rectangle, uniform radiance across ring: PPF_total / (A_total * π)
        A_total = STRIP_W_M * total_len
        strip_rad_by_ring[L] = _radiance_from_ppf((total_w * eff) * strip_ppe, A_total) if total_len > 0 else 0.0

    with out_emitters.open("w") as fh:
        fh.write("# COB + per-ring strip emitters in µmol/s/sr/m² (Lambertian)\n")
        ...
        fh.write(f"# Strips: width={STRIP_W_M:.3f} m  PPE={strip_ppe:.3f} µmol/J  mode={meta.get('strip_mode')}\n\n")

        # Per-ring COB materials
        for L in range(rings_local):
            rad = float(cob_rad_by_ring.get(L, 0.0))
            fh.write(f"void glow cob_L{L}\n0\n0\n4 {rad:.6f} {rad:.6f} {rad:.6f} {glow_maxrad:.3f}\n\n")

        # Per-ring strip materials (separate solver channels)
        for L in range(rings_local):
            rad = float(strip_rad_by_ring.get(L, 0.0))
            fh.write(f"void glow strip_L{L}\n0\n0\n4 {rad:.6f} {rad:.6f} {rad:.6f} {glow_maxrad:.3f}\n\n")
    ...
    for L in range(rings_local):
        ...
        per_ring_lines.append(
            ...
            f"STRIP {strip_w_in_L:8.2f} W in ({strip_w_eff_L:8.2f} W eff) "
    ...
    summ.write_text(
        ...
        + f"  strip mode  : {meta.get('strip_mode')}\n"
        + f"  strip frac  : {float(meta.get('strip_w_fraction_of_cob', 0.0)):.4f} (of COB ring watts)\n"
        + f"  COB PPE     : {cob_ppe:.3f} µmol/J\n"
        + f"  STRIP PPE   : {strip_ppe:.3f} µmol/J\n"
        + f"  COB watts   : input ≈ {total_cob_w_in:.1f} W  effective ≈ {total_cob_w_eff:.1f} W\n"
        + f"  strip watts : input ≈ {total_strip_w_in:.1f} W  effective ≈ {total_strip_w_eff:.1f} W\n"
    ...
    print(f"  strip mode  : {meta.get('strip_mode')}")
    print(f"  COB watts   : input ≈ {total_cob_w_in:.1f} W  effective ≈ {total_cob_w_eff:.1f} W")
    print(f"  strip watts : input ≈ {total_strip_w_in:.1f} W  effective ≈ {total_strip_w_eff:.1f} W")
    print(f"  total photons ≈ {total_ppf:.0f} µmol/s  (COB {total_cob_ppf:.0f} + strips {total_strip_ppf:.0f})")
    if per_ring_lines:
        print("  per-ring (COB + STRIP):")
