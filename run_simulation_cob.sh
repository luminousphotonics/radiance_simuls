#!/usr/bin/env bash
# run_simulation_cob.sh • COB + perimeter strip PPFD (single-pass, µmol units)

set -euo pipefail
echo
echo "--- COB PPFD Simulation (single-pass, PPFD units) ---"
...
# 1) Build emitters (writes $OUTDIR/emitters_cob_ALL_umol.rad + overlay/json)
"$PY" "$ROOT/generate_emitters_cob.py"

# 2) Sanity
[[ -f "$ROOM" ]] || { echo "ERROR: $ROOM not found."; exit 1; }
[[ -f "$OUTDIR/emitters_cob_ALL_umol.rad" ]] || { echo "ERROR: $OUTDIR/emitters_cob_ALL_umol.rad missing."; exit 1; }

echo "Geometry files:"
echo "  • $ROOM"
echo "Emitters:"
echo "  • $OUTDIR/emitters_cob_ALL_umol.rad"

# 3) Radiance presets
AMBCACHE_TAG="main_$$"
if [[ "${COB_BASIS_MODE:-0}" == "1" ]]; then
  AMBCACHE_TAG="basis_${COB_BASIS_RING:-x}_$$"
fi
AMBCACHE="$ROOT/.radcache/amb_${AMBCACHE_TAG}"
...
if [[ "${COB_BASIS_MODE:-0}" == "1" ]]; then NTHREADS=1; fi

# Build room.rad for current LENGTH_M, WIDTH_M, HEIGHT_M
"$PY" "$ROOT/generate_room.py" > "$ROOM"

# Build sensor grid for current room dimensions, using COB fixture footprint for inset.
# For COB mode we want the heatmap to cover the full grow space footprint.
# Use a tiny wall inset to avoid sampling directly on wall surfaces; do NOT shrink by module footprint.
...
oconv -f "$ROOM" "$OUTDIR/emitters_cob_ALL_umol.rad" > "$OCT"
...
if not os.path.exists(ppfd_path):
    ppfd_path = "ppfd_map.txt"

vals = []
with open(ppfd_path, "r", encoding="utf-8") as f:
    for line in f:
        s = line.strip()
...
ppfd = np.asarray(vals, dtype=float)
...
emitted_ppf = None
try:
    summ_path = os.path.join(os.environ.get("ROOT", "."), "ies_sources", "cob_summary.txt")
